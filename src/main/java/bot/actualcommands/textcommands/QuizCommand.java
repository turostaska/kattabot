...
public class QuizCommand implements ICommand {
    ...
    private void sendQuestion() {
        ...
        var embed = UtilsKt.createBasicReactionEmbed(currentQuestion.question, new ArrayList<>(answers), language);
    ...
    private List<Categories.Category> getCategories() {
        String receivedContent = UtilsKt.getJsonFromAPI("https://opentdb.com/api_category.php");
    ...
    private QuestionResponse.Question requestQuestion() {
        ...
        String receivedContent = UtilsKt.getJsonFromAPI(url);

        Integer responseCode = Integer.parseInt(UtilsKt.getJsonPropertyValue(receivedContent, "response_code"));

        if (responseCode > 0) {
            requestToken();
            url = url.substring(0, url.lastIndexOf("=") + 1) + questionToken;
            receivedContent = UtilsKt.getJsonFromAPI(url);
        }
    ...
    private void updateScoreInRemoteDatabase() {
        ...
        int status = UtilsKt.sendHttpRequest(updateUrl, "POST", jsonToSend);
    ...
    private void requestToken() {
        String json = UtilsKt.getJsonFromAPI("https://opentdb.com/api_token.php?command=request");

        questionToken = UtilsKt.getJsonPropertyValue(json, "token");
